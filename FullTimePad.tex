...
\usepackage{subcaption} % for putting graphs side-by-side
\usepackage{pdfpages}
...
\begin{figure}[H]
    \caption{Collision Rate (\%) vs. Inaccuracy ($n$)}
    \centering
    \begin{subfigure}{0.48\textwidth}
        \centering
	    \hspace{-3cm}  % Move the first figure closer to the left side margin
		\begin{tikzpicture}
			\begin{axis}[
				xlabel={Inaccuracy ($n$)},
				ylabel={Collision Rate},
				title={Full-Time-Pad 1.0}
			]
			    \addplot[line width=2pt] coordinates {
		...
		\end{tikzpicture}
    \end{subfigure}
	 \hspace{-0.9cm}  % Decrease space between subfigures
    \begin{subfigure}{0.48\textwidth}
        \centering
		\begin{tikzpicture}
			\begin{axis}[
...
These graphs demonstrates how lower accuracy of comparison results in higher collision rate (For both SHA-256 and Full-Time-Pad). The points for this graph is evaluated by running \texttt{test/significant\_perm\_byte -d -version\_number}. The consistent non-volatile increase in the numbers are a good thing.
